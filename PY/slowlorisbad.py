...
def run(args):
    ...
    print("Attacking %s with %s sockets" % (host, socket_count), 'info')

    print("Creating sockets...", 'info')
    for i in range(socket_count):
        try:
            print("Creating socket number %s" % i, 'debug')
    ...
    while True:
        print("Sending keep-alive headers... Socket count: %s" % len(list_of_sockets), 'info')
